 ...
 * @param {string} private_key_json
 * @returns {Uint8Array}
 */
export function decrypt_file(ciphertext_with_nonce, encrypted_key, private_key_json) {
    const ptr0 = passArray8ToWasm0(ciphertext_with_nonce, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passArray8ToWasm0(encrypted_key, wasm.__wbindgen_malloc);
    const len1 = WASM_VECTOR_LEN;
    const ptr2 = passStringToWasm0(private_key_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len2 = WASM_VECTOR_LEN;
    const ret = wasm.decrypt_file(ptr0, len0, ptr1, len1, ptr2, len2);
    if (ret[3]) {
 ...
 * @returns {any}
 */
export function encrypt_file(file, public_key_json) {
    const ptr0 = passArray8ToWasm0(file, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passStringToWasm0(public_key_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len1 = WASM_VECTOR_LEN;
 ...
 * @returns {string}
 */
export function generate_keys(bits) {
    let deferred2_0;
    let deferred2_1;
    try {
        const ret = wasm.generate_keys(isLikeNone(bits) ? 0x100000001 : (bits) >>> 0);
        var ptr1 = ret[0];
        var len1 = ret[1];
        if (ret[3]) {
            ptr1 = 0; len1 = 0;
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('rsa_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
