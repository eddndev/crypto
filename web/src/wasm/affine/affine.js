 ...
 * @returns {string}
 */
export function compute(alpha, beta, n) {
    let deferred2_0;
    let deferred2_1;
    try {
        const ret = wasm.compute(alpha, beta, n);
        var ptr1 = ret[0];
        var len1 = ret[1];
        if (ret[3]) {
            ptr1 = 0; len1 = 0;
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('affine_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
